...
local Window = Fluent:CreateWindow({
    Title = "Falfao hub V4 [Back]",
    SubTitle = " by ★ AnhKhoa2279  [Owner NazuX]",
    Search = true,
    TabWidth = 185,
...
Info:AddButton({
    Title = "NazuX hub [My Main Script]",
    Description = "Script by ★ AnhKhoa2279 ",
    Callback = function()
...
Main:AddSection("Fishing [Beta]")

FishingState = {
    Enabled = false,
    CurrentRod = "Basic Fishing Rod",
    FishingSpeed = 1,
    TotalFish = 0,
    StartTime = 0,
    IsFishing = false
...
-- Dropdown chọn cần câu
RodDropdown = Main:AddDropdown("RodDropdown", {
    ...
    Default = "Basic Fishing Rod",
    Multi = false,
})

RodDropdown:OnChanged(function(Value)
    FishingState.CurrentRod = Value
end)

-- Slider tốc độ
peedSlider = Main:AddSlider("SpeedSlider", {
    Title = "Fishing Speed",
    Description = "Adjust fishing speed",
    Default = 1,
    Min = 0.5,
    Max = 3,
    Rounding = 1,
    Callback = function(Value)
        FishingState.FishingSpeed = Value
    end
})

StatusLabel = Main:AddParagraph({
    Title = "Current Status",
    Content = "🔴 Inactive"
})

FishCountLabel = Main:AddParagraph({
    Title = "Fish Caught",
    Content = "0"
})

-- Toggle Auto Fishing
AutoToggle = Main:AddToggle("AutoToggle", {
    Title = "Enable Auto Fishing",
    Default = false,
    Callback = function(Value)
        FishingState.Enabled = Value
        if Value then
            StartAutoFishing()
            Fluent:Notify({
...
-- Nút câu cá thủ công
Main:AddButton({
    Title = "Manual Fish Once",
    Description = "Single fishing attempt",
    Callback = function()
        if not FishingState.IsFishing then
            AttemptFishing()
        else
            Fluent:Notify({
                Title = "Already Fishing",
                Content = "Wait for current fishing to complete!",
                Duration = 2
            })
        end
    end
...
function EquipFishingRod()
    ...
    for _, tool in pairs(backpack:GetChildren()) do
        if tool:IsA("Tool") then
            ...
            if string.find(toolName, "fishing") or string.find(toolName, "rod") then
...
function AttemptFishing()
    ...
    StatusLabel:Set({Content = "🎣 Casting line..."})
    
    if not EquipFishingRod() then
        StatusLabel:Set({Content = "❌ No fishing rod"})
        FishingState.IsFishing = false
        return
    end
    
    spawn(function()
        local waitTime = 3 / FishingState.FishingSpeed
        
        for i = waitTime, 1, -1 do
            if not FishingState.Enabled and not FishingState.IsFishing then break end
            StatusLabel:Set({Content = "🎣 Fishing... " .. i .. "s"})
            wait(1)
        end
        
        if FishingState.IsFishing then
            if math.random(1, 100) > 20 then
                FishingState.TotalFish = FishingState.TotalFish + 1
                StatusLabel:Set({Content = "✅ Fish caught! Total: " .. FishingState.TotalFish})
                FishCountLabel:Set({Content = tostring(FishingState.TotalFish)})
            else
                StatusLabel:Set({Content = "❌ Fish escaped"})
            end
            
            FishingState.IsFishing = false
            
            if FishingState.Enabled then
                StatusLabel:Set({Content = "🔄 Ready for next cast"})
            else
                StatusLabel:Set({Content = "🔴 Inactive"})
            end
        end
    end)
end
...
function StartAutoFishing()
    FishingState.StartTime = os.time()
    
    spawn(function()
        while FishingState.Enabled do
            if not FishingState.IsFishing then
                AttemptFishing()
            end
            wait(0.5)
...
-- Keybind để toggle auto fishing
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if gameProcessed then return end
    
    if input.KeyCode == Enum.KeyCode.F then
        AutoToggle:Set(not FishingState.Enabled)
    end
end)
