...
if executor then
    if string.find(executor, "Delta") or string.find(executor, "Krnl") or string.find(executor, "Arceus") or string.find(executor,"Swift") or string.find(executor, "Awp") or string.find(executor, "Volcano")  or string.find(executor, "Argon") or string.find(executor, "Macsploit") or string.find(executor, "Potassium") or string.find(executor, "CodeX") or string.find(executor, "Velocity") or string.find(executor, "Romix") then
        print("Supported Executor!")
    elseif string.find(executor, "Xeno") then
        game.Players.LocalPlayer:Kick("Not support This Exploit 🔴, please Go My Discord to get a new exploit : https://discord.gg/3h4ZKsRcpn")
...
elseif a>=2700 and a<=2724 then
    Mon="High Disciple"
    Qdata=2; Qname="SubmergedQuest3"; NameMon="Ocean Prophet"
...
local MAX_SYSTEMS = 250
local ATTACK_RANGE = 1000

-- Cache các event
local attackEvent, hitEvent
local function GetEvents()
    if not attackEvent then
        local storage = game:GetService("ReplicatedStorage")
        attackEvent = storage:WaitForChild("Modules"):WaitForChild("Net"):WaitForChild("RE"):WaitForChild("RegisterAttack")
        hitEvent = storage:WaitForChild("Modules"):WaitForChild("Net"):WaitForChild("RE"):WaitForChild("RegisterHit")
    end
    return attackEvent, hitEvent
end
...
-- Lấy targets
local function GetTargetsInRange(character)
    local targets = {}
    ...
    for _, enemy in ipairs(workspace.Enemies:GetChildren()) do
        ...
        if hrp and head and IsEntityAlive(enemy) then
    ...
    for _, otherPlayer in ipairs(game.Players:GetPlayers()) do
        if otherPlayer ~= player and otherPlayer.Character then
            ...
            if hrp and head and IsEntityAlive(char) then
                local distance = (hrp.Position - playerPos).Magnitude
                if distance <= ATTACK_RANGE then
                    table.insert(targets, {char, head})
                end
            end
        end
    end
...
-- Hàm đánh chính - tốc độ vừa phải
function AttackNormal()
    local character = player.Character
    if not character then return end
    
    local targets = GetTargetsInRange(character)
    if #targets > 0 then
        local attEvent, hitEvt = GetEvents()
        
        -- Tấn công 3 lần mỗi lần gọi (vừa phải)
        for i = 1, 3 do
            task.spawn(function()
                pcall(function()
                    attEvent:FireServer(0)
                    hitEvt:FireServer(targets[1][2], targets, MAX_DAMAGE)
                end)
            end)
            task.wait(0)  -- Delay vừa phải
        end
    end
end

-- Tạo hệ thống tấn công
for i = 250, MAX_SYSTEMS do
    spawn(function()
        while task.wait(250) do  -- Tốc độ vừa phải
            pcall(AttackNormal)
        end
    end)
end
...
spawn(function()
    ...
    -- Tìm remote
    for _, v in next, ({game.ReplicatedStorage.Util, game.ReplicatedStorage.Common, game.ReplicatedStorage.Remotes}) do
    ...
    while task.wait(0.2) do  -- Tốc độ chậm hơn
        ...
        if root then
            ...
            for _, x in ipairs({workspace.Enemies, workspace.Characters}) do
                for _, v in ipairs(x:GetChildren()) do
                    if v ~= char then
                        ...
                        if hrp and head and IsEntityAlive(v) and (hrp.Position - playerPos).Magnitude <= ATTACK_RANGE then
            ...
            if #parts > 0 then
                pcall(function()
                    ...
                    -- Chỉ gửi 1 lần
                    netModule:RemoteEvent("RegisterHit", true)
                    game.ReplicatedStorage.Modules.Net["RE/RegisterAttack"]:FireServer()
                    game.ReplicatedStorage.Modules.Net["RE/RegisterHit"]:FireServer(parts[1][2], parts, {Damage = MAX_DAMAGE}, "NORMAL")
                    
                    -- Gửi remote
                    if remote then
                        cloneref(remote):FireServer("RE/RegisterHit", MAX_DAMAGE, parts[1][2], parts, MAX_DAMAGE)
                    end
                end)
            end
        end
...
spawn(OverrideDamage)
-- Notify Roblox
game.StarterGui:SetCore("SendNotification", {
    Title = "NazuX hub";
    Text = "Success";
    Icon = "rbxassetid://74725622037659";
})
-- Create Toogle Ui
...
ScreenGui.Name = "NazuX toggle Banana"
...
ImageLabel.Image = "rbxassetid://74725622037659"
ImageLabel.ImageRectOffset = Vector2.new(4, 884)
...
-- Hiệu ứng hover Fluent
mainButton.MouseEnter:Connect(function()
 isHovering = true
 TweenService:Create(mainButton, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
-- Hiệu ứng toggle Fluent
mainButton.MouseButton1Click:Connect(function()
 isToggled = not isToggled
 print("Toggle:", isToggled)
 
 -- Hiệu ứng scale Fluent
 local scaleTween = TweenService:Create(mainButton, TweenInfo.new(0.15, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
  Size = UDim2.new(0, 55, 0, 55)
 })
 
 local scaleBackTween = TweenService:Create(mainButton, TweenInfo.new(0.15, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
Size = UDim2.new(0, 60, 0, 60)
 })
 
 -- Hiệu ứng blur Fluent
 local blurTween = TweenService:Create(BlurEffect, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
  Size = isToggled and 8 or 0
 })
 
 -- Hiệu ứng icon scale
 local iconScaleTween = TweenService:Create(ImageLabel, TweenInfo.new(0.15, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
  Size = isToggled and UDim2.new(0, 30, 0, 30) or UDim2.new(0, 35, 0, 35)
 })
...
-- Double click để reset vị trí
local lastClickTime = 0
mainButton.MouseButton1Click:Connect(function()
 local currentTime = tick()
 if currentTime - lastClickTime < 0.3 then
  -- Double click: reset vị trí
  TweenService:Create(mainButton, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
-- local Fluent
local Fluent = loadstring(game:HttpGet("https://raw.githubusercontent.com/NguyenAnhKhoaX/Fluent/refs/heads/main/UI"))()
local SaveManager = loadstring(game:HttpGet("https://raw.githubusercontent.com/dawid-scripts/Fluent/master/Addons/SaveManager.lua"))()
local InterfaceManager = loadstring(game:HttpGet("https://raw.githubusercontent.com/dawid-scripts/Fluent/master/Addons/InterfaceManager.lua"))()
-- local window fluent
local Window = Fluent:CreateWindow({
    Title = "NazuX hub | Blox Fruits [Free Version]",
...
Info = Window:AddTab({ Title = "Tab Credits", Icon = "user" })
Info:AddSection("Update")
Info:AddParagraph({
    Title = "What Update ?",
    Content = "This Update Has : Just Script back and added Script Faifao hub"
})
Info:AddParagraph({
    Title = "Next Update",
    Content = "Next Update is : Change New UI [I think so]"
})
Info:AddSection("Faifao Back")
Info:AddButton({
    Title = "FaiFao hub Back",
    Description = "Script Crack But All features It's Me",
    Callback = function()
        
    end
})
Info:AddSection("Get News My Script")
Info:AddButton({
    Title = "Join TrongNguyen Discord",
    Description = "",
    Callback = function()
        Window:Dialog({
            Title = "NazuX hub",
            Content = "Do You Want Copy Link ?",
            Buttons = {
                {
                    Title = "Yes",
                    Callback = function()
                        setclipboard("https://discord.gg/V5QT3TPy")
                    end
                },
                {
                    Title = "No",
            ...
            Buttons = {
                {
                    Title = "Yes",
                    Callback = function()
                        setclipboard("https://www.youtube.com/@Real_AnhKhoa_2279")
                    end
                },
                {
                    Title = "No",
...
Info:AddSection("Exploit Woking")
Info:AddParagraph({
    Title = "Exploit :",
    Content = "Support All Exploit not Solara, Nexur, TNG, JJSploit And SynapseX"
})
Status = Window:AddTab({ Title = "Tab Status And Sever", Icon = "rbxassetid://10709773755" })
Time = Status:AddParagraph({
    Title = "Time Your Location",
    Content = ""
})
function UpdateOS()
...
function UpdateTime()
    ...
    Timmessss:SetDesc(Hour.." Hour "..Minute.." Minute "..Second.." Second ")
end
spawn(function()
    while true do
        UpdateTime()
...
FpsParagraph = Status:AddParagraph({
	Title = "Fps",
	Desc = "0",
	Image = "monitor",
	ImageSize = 20
...
PingParagraph = Status:AddParagraph({
	Title = "Ping",
	Desc = "0",
	Image = "signal",
	ImageSize = 20
...
__indexPlayer = Status:AddParagraph({Title = "Player",Content = ""})
spawn(function()
  while wait(Sec) do
    pcall(function()
...
EliteHunterParagraph = Status:AddParagraph({
	Title = "Elite Hunter",
	Content = "Status: "
});
spawn(function()
	while wait(0.2) do
		pcall(function()
			if (game:GetService("ReplicatedStorage")):FindFirstChild("Diablo") or (game:GetService("ReplicatedStorage")):FindFirstChild("Deandre") or (game:GetService("ReplicatedStorage")):FindFirstChild("Urban") or (game:GetService("Workspace")).Enemies:FindFirstChild("Diablo") or (game:GetService("Workspace")).Enemies:FindFirstChild("Deandre") or (game:GetService("Workspace")).Enemies:FindFirstChild("Urban") then
				EliteHunterParagraph:SetDesc("Status: ✅️");
			else
				EliteHunterParagraph:SetDesc("Status: ❌️");
			end;
		end);
...
MobCakePrince = Status:AddParagraph({
    Title = "Katakuri V1",
    Content = "Status: "
})
spawn(function()
...
TyrantStatus = Status:AddParagraph({
...
CheckDoughKing = Status:AddParagraph({
    Title = "Katakuri V2",
    Content = "Status: "
})
spawn(function()
...
Status:AddSection("Status Island")
CPrehistoriccheck = Status:AddParagraph({
...
FrozenIsland = Status:AddParagraph({
...
Kitsunecheck = Status:AddParagraph({
...
Miragecheck = Status:AddParagraph({
    Title = "Mirage Island",
    Content = "Status: "
})
local previousStatus = ""
...
Status:AddSection("Other")
LegendarySword = Status:AddParagraph({
...
SPYING = Status:AddParagraph({Title = " Spy Status ",Content = ""})
spawn(function()
  while wait(.2) do
    pcall(function()
...
Status:AddSection("Sever")
Input = Status:AddInput("Input", {
     Title = "Input Job Id",
     Default = "",
     Placeholder = "",
     Numeric = false,
...
Toggle = Status:AddToggle("Toggle", {Title = "Spam Join", Default = false })
...
Status:AddButton({
    Title = "Hop Sever VIP",
    Callback = function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/NguyenAnhKhoaX/AnhKhoa2279/refs/heads/main/OpenSource"))()
    end
})
local lastTeleportTime = 0
...
_Weapon = {"Melee","Sword","Blox Fruit","Gun"}
Weapon_Config = Main:AddDropdown("Weapon_Config",{Title = "Select Weapon",Values = _Weapon,Multi = false,Default = 1})
...
FarmLevel = Main:AddToggle("FarmLevel", {Title = "Auto Level Farm", Description = "", Default = false})
...
ClosetMons = Main:AddToggle("ClosetMons", {Title = "Auto Farm Nearest", Description = "", Default = false})
...
Q = Main:AddToggle("Q", {Title = "Auto Bones Farm", Description = "", Default = false})
...
CastleRaids = Main:AddToggle("CastleRaids", {Title = "Auto Pirate Raid", Description = "", Default = false})
...
FactoryRaids = Main:AddToggle("FactoryRaids", {Title = "Auto Factory Raid", Description = "", Default = false})
...
Test = Main:AddDropdown("Test",{Title = "Select Your Farm Material",Values = MaterialList,Multi = false,Default = nil})
Test:OnChanged(function(Value)
  getgenv().SelectMaterial = Value
end)
Toggle = Main:AddToggle("Toggle", {Title = "Auto Materials Farm", Description = "", Default = false})
Toggle:OnChanged(function(Value)
...
Main:AddSection("Quest")
Bartilo = Main:AddToggle("Bartilo", {Title = "Auto Bartilo Quest", Description = "", Default = false})
...
CitizenQ = Main:AddToggle("CitizenQ", {Title = "Auto Citizen Quest", Description = "", Default = false})
...
MasteryFruits = Main:AddToggle("MasteryFruits", {Title = "Auto Mastery Fruits", Description = "", Default = false})
...
MasteryGun = Main:AddToggle("MasteryGun", {Title = "Auto Mastery Gun", Description = "", Default = false})
...
Island = Window:AddTab({ Title = "Tab Teleport", Icon = "rbxassetid://10734910680" })
Island:AddSection("Teleport Sea")
...
Island:AddSection("Teleport Island")
Location = {}
for i,v in pairs(workspace["_WorldOrigin"].Locations:GetChildren()) do  
  table.insert(Location ,v.Name)
end
Travelllll = Island:AddDropdown("Travelllll",{Title = "Select Your Island",Values = Location,Multi = false,Default = 1})
Travelllll:OnChanged(function(Value)
  _G.Island = Value
end)
GoIsland = Island:AddToggle("GoIsland", {Title = "Auto Teleport", Description = "", Default = false})
...
Muti = Window:AddTab({ Title = "Tab Stack Farming", Icon = "rbxassetid://10709769508" })
Muti:AddSection("Auto Get Melee")
SuperHuman = Muti:AddToggle("SuperHuman", {Title = "Auto Superhuman", Description = "", Default = false})
...
DeathStep = Muti:AddToggle("DeathStep", {Title = "Auto DeathStep", Description = "", Default = false})
...
SharkManV2 = Muti:AddToggle("SharkManV2", {Title = "Auto Sharkman Karate", Description = "", Default = false})
...
ElectricClaw = Muti:AddToggle("ElectricClaw", {Title = "Auto ElectricClaw", Description = "", Default = false})
...
DragonTalon = Muti:AddToggle("DragonTalon", {Title = "Auto DragonTalon", Description = "", Default = false})
...
Godhuman = Muti:AddToggle("Godhuman", {Title = "Auto Godhuman", Description = "", Default = false})
...
SanguineArt = Muti:AddToggle("SanguineArt", {Title = "Auto SanguineArt", Description = "", Default = false})
...
Muti:AddSection("Boss")
Q = Muti:AddToggle("Q", {Title = "Auto Rip_indra", Description = "", Default = false})
Q:OnChanged(function(Value)
...
Q = Muti:AddToggle("Q", {Title = "Auto Elite", Description = "", Default = false})
Q:OnChanged(function(Value)
...
Q = Muti:AddToggle("Q", {Title = "Auto Cake Prince", Description = "", Default = false})
Q:OnChanged(function(Value)
...
Q = Muti:AddToggle("Q", {Title = "Auto Soul Reaper", Description = "", Default = false})
Q:OnChanged(function(Value)
...
Muti:AddSection("Chest Farm")
Chest = Muti:AddToggle("Chest", {Title = "Auto Farm Chest", Description = "", Default = false})
...
CUP = Muti:AddToggle("CUP", {Title = "Stop when got A CUP", Description = "", Default = true})
CUP:OnChanged(function(Value)
...
Items = Window:AddTab({ Title = "Tab Get Items", Icon = "box" })
Items:AddSection("Auto Sword")
Q = Items:AddToggle("Q", {Title = "Auto Tushita Sword", Description = "Need Rip_indra", Default = false})
Q:OnChanged(function(Value)
...
Q = Items:AddToggle("Q", {Title = "Auto Yama Sword", Description = "Need 30 Elite", Default = false})
Q:OnChanged(function(Value)
  _G.Auto_Yama = Value
end)
spawn(function()
...
Q = Items:AddToggle("Q", {Title = "Auto Quests 1 [Cursed Dual Katana]", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.CDK_YM = Value
end)
spawn(function()
...
Q = Items:AddToggle("Q", {Title = "Auto Quests 2 [Cursed Dual Katana]", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.CDK_TS = Value
end)
spawn(function()
...
Q = Items:AddToggle("Q", {Title = "Auto Get Cursed Dual Katana", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.CDK = Value
end)
spawn(function()    
...
Q = Items:AddToggle("Q", {Title = "Auto Get Bisento", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.Greybeard = Value
end)
spawn(function()
...
Q = Items:AddToggle("Q", {Title = "Auto Get Saber", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.AutoSaber = Value
end)
spawn(function()
...
Q = Items:AddToggle("Q", {Title = "Auto Get Saw", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.AutoSaw = Value
end)
spawn(function()
...
Q = Items:AddToggle("Q", {Title = "Auto Get Warden", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.WardenBoss = Value
end)
spawn(function()
...
Q = Items:AddToggle("Q", {Title = "Auto Get Long", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.LongsWord = Value
end)
spawn(function()
...
Q = Items:AddToggle("Q", {Title = "Auto Get Dragon Trident", Description = "", Default = false})
Q:OnChanged(function(Value)
...
Q = Items:AddToggle("Q", {Title = "Auto Get Rengoku", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.IceBossRen = Value
end)
spawn(function()
...
Q = Items:AddToggle("Q", {Title = "Auto Pole V1", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.AutoPole = Value
end)
spawn(function()
...
Q = Items:AddToggle("Q", {Title = "Auto Pole V2", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.AutoPoleV2 = Value
end)
spawn(function()
...
Q = Items:AddToggle("Q", {Title = "Auto Get Midnight Blade", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.AutoEcBoss = Value
end)
spawn(function()
...
Q = Items:AddToggle("Q", {Title = "Auto Twin Hooks", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.TwinHook = Value
end)
spawn(function()
...
Items:AddSection("Auto Gun")
Q = Items:AddToggle("Q", {Title = "Auto Get Serpent Bow", Description = "", Default = false})
Q:OnChanged(function(Value)
...
Q = Items:AddToggle("Q", {Title = "Auto Skull Guitar", Description = "", Default = false})
Q:OnChanged(function(Value)
...
Items:AddSection("Auto Accessory")
Q = Items:AddToggle("Q", {Title = "Auto Lei Accessory", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.AutoKilo = Value
end)
spawn(function()
...
Shop = Window:AddTab({ Title = "Tab Shop", Icon = "rbxassetid://10734952479" })
...
Shop:AddButton({
    Title = "Draco Race [Soon]",
    Callback = function()

    end
})
Stats = Window:AddTab({ Title = "Tab Stats", Icon = "rbxassetid://10734923868" })
Stats:AddSection("Auto Stats")
StatusSelect = Stats:AddSlider("Slider", {
    Title = "Select your Stats Value",
    Description = "",
    Default = 2,
    Min = 0,
    Max = 2800,
...
StatsUpg = Stats:AddToggle("StatsUpg", {Title = "Auto Melee", Description = "", Default = false})
...
StatsUpg = Stats:AddToggle("StatsUpg", {Title = "Auto Swords", Description = "", Default = false})
StatsUpg:OnChanged(function(Value)
  _G.Auto_Sword = Value
end)
spawn(function()
...
StatsUpg = Stats:AddToggle("StatsUpg", {Title = "Auto Gun", Description = "", Default = false})
StatsUpg:OnChanged(function(Value)
  _G.Auto_Gun = Value
end)
spawn(function()
...
StatsUpg = Stats:AddToggle("StatsUpg", {Title = "Auto Fruits", Description = "", Default = false})
StatsUpg:OnChanged(function(Value)
...
StatsUpg = Stats:AddToggle("StatsUpg", {Title = "Auto Defense", Description = "", Default = false})
StatsUpg:OnChanged(function(Value)
...
RaF = Window:AddTab({ Title = "Tab Fruits + Raid", Icon = "apple" })
RaF:AddSection("Fruits")
StoredF = RaF:AddToggle("StoredF", {Title = "Auto Store Fruit", Description = "Automatic store devil fruit", Default = false})
...
TwF = RaF:AddToggle("TwF", {Title = "Auto Tween to Fruit", Description = "", Default = false})
...
RandomFF = RaF:AddToggle("RandomFF", {Title = "Auto Random Fruit", Description = "", Default = false})
...
RaF:AddSection("Raid")
Q = RaF:AddDropdown("Q",{Title = "Select Core",Values = DungeonTables,Multi = false,Default = 1})
Q:OnChanged(function(Value)
  _G.SelectChip = Value
end)
Q = RaF:AddToggle("Q", {Title = "Auto Select This Core", Description = "", Default = false})
Q:OnChanged(function(Value)
...
RaF:AddButton({Title = "Buy Dungeon Chips with Beli", Description = "",
Callback = function()
  if not GetBP("Special Microchip") then replicated.Remotes.CommF_:InvokeServer("RaidsNpc","Select",_G.SelectChip) end
end})
RaF:AddButton({Title = "Buy Dungeon Chips With Low Fruits", Description = "",
Callback = function()
...
Q = RaF:AddToggle("Q", {Title = "Auto Raid", Description = "", Default = false})
Q:OnChanged(function(Value)
...
Q = RaF:AddToggle("Q", {Title = "Auto Complete Raid [Safety]", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.Raiding = Value
end)
spawn(function()
...
Q = RaF:AddToggle("Q", {Title = "Kill Aura", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.KillH = Value
end)
spawn(function()
...
Q = RaF:AddToggle("Q", {Title = "Auto Next Island", Description = "", Default = false})
Q:OnChanged(function(Value)
  NextIs = Value
end)
spawn(function()
...
Q = RaF:AddToggle("Q", {Title = "Auto Awakening Fruits", Description = "", Default = false})
Q:OnChanged(function(Value)
...
Sea = Window:AddTab({ Title = "Tab Sea Event", Icon = "rbxassetid://10723427494" })
Sea:AddSection("Settings Boats")
...
Q = Sea:AddDropdown("Q",{Title = "Select Your Boats",Values = ListSeaBoat,Multi = false,Default = 1})
Q:OnChanged(function(Value)
  _G.SelectedBoat = Value
end)
Sea:AddButton({Title = "Buy Boats", Description = "Buy the select boats",
Callback = function()
  replicated.Remotes.CommF_:InvokeServer("BuyBoat",_G.SelectedBoat)
end})
ListSeaZone={"Lv 1","Lv 2","Lv 3","Lv 4","Lv 5","Lv 6"}
Q = Sea:AddDropdown("Q",{Title = "Select Sea Level",Values = ListSeaZone,Multi = false,Default = 1})
Q:OnChanged(function(Value)
  _G.DangerSc = Value
end)
Q = Sea:AddToggle("Q", {Title = "Auto Anti Break Boats", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.SailBoats = Value
end)
spawn(function()
...
Sea:AddSection("Auto Kill")
Q = Sea:AddToggle("Q", {Title = "Auto Sea Beast", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.SeaBeast1 = Value
end)
Q = Sea:AddToggle("Q", {Title = "Auto Terror Shark", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.TerrorShark = Value
end)
Q = Sea:AddToggle("Q", {Title = "Auto Shark", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.Shark = Value
end)
Q = Sea:AddToggle("Q", {Title = "Auto Piranha", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.Piranha = Value
end)
Q = Sea:AddToggle("Q", {Title = "Auto Pirate Big Boats", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.PGB = Value
end)
Q = Sea:AddToggle("Q", {Title = "Auto Pirate Ghost Boats", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.HCM = Value
end)
spawn(function()
...
Sea:AddSection("Volcano Event")
Sea:AddButton({
	Title = "Delete Lava In Volcano Island",
	Callback = function()
...
Toggle = Sea:AddToggle("MyToggle", {Title = "Auto Teleport Volcano Island", Default = false })
Toggle:OnChanged(function(Value)
...
Q = Sea:AddToggle("Q", {Title = "Auto Teleport Kitsune Island", Description = "", Default = false})
Q:OnChanged(function(Value)
...
Q = Sea:AddToggle("Q", {Title = "Auto Farm Soul Blue", Description = "", Default = false})
Q:OnChanged(function(Value)
...
Q = Sea:AddToggle("Q", {Title = "Auto Trade Soul Blue", Description = "", Default = false})
Q:OnChanged(function(Value)
...
Sea:AddButton({Title = "Buy F to NPC Spy", Description = "",
Callback = function()
  replicated:WaitForChild("Remotes"):WaitForChild("CommF_"):InvokeServer("InfoLeviathan","2")
end})
Q = Sea:AddToggle("Q", {Title = "Auto Teleport Leviathan Island", Description = "turn on for teleport to frozen dimension and start the leviathan gate", Default = false})
Q:OnChanged(function(Value)
  _G.FrozenTP = Value
end)
spawn(function()
...
Q = Sea:AddToggle("Q", {Title = "Auto Attack Leviathan", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.Leviathan1 = Value
end)
Race = Window:AddTab({ Title = "Tab Upgrade Race V4", Icon = "arrow-big-right" })
Q = Race:AddToggle("Q", {Title = "Auto Pull Lever", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.Lver = Value
end)
spawn(function()
...
Q = Race:AddToggle("Q", {Title = "Auto Train V4", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.AcientOne = Value
end)
spawn(function()
...
Race:AddButton({Title = "Teleport to Ancient Clock", Description = "",
Callback = function()
  notween(CFrame.new(29549, 15069, -88))
end})
Race:AddButton({Title = "Teleport to Ancient One", Description = "",
Callback = function()
  notween(CFrame.new(28981.552734375, 14888.4267578125, - 120.245849609375))
end})
Race:AddButton({Title = "Teleport to Temple of Time", Description = "",
Callback = function()
  game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("requestEntrance",Vector3.new(28286.35546875, 14895.3017578125, 102.62469482421875))
end})
Q = Race:AddToggle("Q", {Title = "Auto Teleport to Race Doors", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.TPDoor = Value
end)
spawn(function()
...
Q = Race:AddToggle("Q", {Title = "Auto Complete Trial Race", Description = "", Default = false})
Q:OnChanged(function(Value)
...
Q = Race:AddToggle("Q", {Title = "Auto Kill Player After Trial", Description = "", Default = false})
Q:OnChanged(function(Value)
  _G.Defeating = Value
end)
spawn(function()
...
Player = Window:AddTab({ Title = "Tab Misc", Icon = "list" })
Player:AddButton({
    Title = "⚠️ Super Fast Attack",
    Description = "",
    Callback = function()
        Window:Dialog({
            Title = "NazuX hub",
            Content = "⚠️ Warning : You Can Crash, You Want Turm On Super Fast Attack ?",
            Buttons = {
                {
                    Title = "Yes",
                    Callback = function()
                        loadstring(game:HttpGet("https://raw.githubusercontent.com/NguyenAnhKhoaX/NazuX/refs/heads/main/Ua"))()
                    end
                },
                {
                    Title = "No",
...
Player:AddSection("Screen")
Toggle = Player:AddToggle("Toggle", {Title = "Black Screen", Default = false })
Toggle:OnChanged(function(Value)
...
Toggle = Player:AddToggle("Toggle", {Title = "White Screen", Default = false })
Toggle:OnChanged(function(Value)
...
InterfaceManager:SetFolder("NazuX hub")
SaveManager:SetFolder("NazuX/BloxFruits")
